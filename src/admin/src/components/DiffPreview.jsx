...
export default function DiffPreview({ additions = [], removals = [], modifications = [] }) {
  if (additions.length === 0 && removals.length === 0 && modifications.length === 0) {
  ...
  return (
    <div className="font-mono text-xs rounded-md border border-border bg-muted/30 p-3 space-y-0.5">
      ...
      {modifications.length > 0 && modifications.map(e => (
        <div key={`mod-${e.cidr}`} className="text-amber-600 dark:text-amber-400">
          ~ {e.cidr}{e.after ? `  # ${e.after}` : ''}
        </div>
      ))}
