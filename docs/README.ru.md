...
```bash
npm run deploy                       # собрать админку, развернуть всё + активировать маршрутизацию
npm run deploy:no-run                # только развернуть файлы (используйте до поднятия туннеля)
npm run deploy:force-routes          # ...и заменить файлы кастомных маршрутов на устройстве копиями из src/configs/
npm run deploy-routes                # быстрая выгрузка только файлов кастомных маршрутов
npm run deploy-routes:keep-remote    # только переприменить уже имеющиеся на устройстве файлы
npm run deploy:admin                 # собрать и развернуть админку с бэкендом, перезапустить сервис
```

Прямые вызовы: `bash src/deploy.sh [--no-run] [--force-routes]`,
`bash src/deploy-routes.sh [--keep-remote]`.

**Кастомные маршруты по умолчанию не перетираются.** Маршруты, добавленные через веб-админку, существуют только на RPi, а полный деплой раньше молча заменял их тем, что лежало в `src/configs/`. Теперь `deploy.sh` оставляет существующий файл на устройстве в покое и сообщает об этом; замену включает `--force-routes`, и перед ней делается бэкап с отметкой времени на самом устройстве. `deploy-routes.sh` по-прежнему выгружает файлы — в этом его назначение, — но тоже сначала делает бэкап и печатает количество маршрутов до и после, предупреждая, если в локальном файле их меньше, чем на устройстве.
