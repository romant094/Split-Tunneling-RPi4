...
- **Российские IP-диапазоны** (загружаются ежедневно с `russia.iplist.opencck.org`) выходят напрямую через шлюз провайдера (Keenetic по адресу `192.168.1.1`).
...
- **Маршрут к VPN-серверу** (`84.32.100.60/32`) всегда сохраняется через провайдера, чтобы не возникало петли маршрутизации.

Устройства в LAN получают RPi как шлюз по умолчанию через DHCP-опцию на Keenetic. Индивидуальная настройка устройств не требуется — разделение трафика полностью прозрачно.

```
Интернет
  ↓
Keenetic (192.168.1.1) — подключение к провайдеру
  ↓ eth0
RPi4 (192.168.1.254) — VPN-шлюз
  ↓
Устройства в LAN (шлюз по умолчанию = 192.168.1.254 через DHCP Keenetic)

Маршрутизация трафика:
  Не-RU → awg0 → AmneziaWG VPN (endpoint: 84.32.100.60:36348)
...
| `KEENETIC_GW` | `192.168.1.1` | Шлюз провайдера (роутер Keenetic) |
| `VPN_SERVER_IP` | `84.32.100.60` | IP-адрес endpoint-а AmneziaWG |
...
После полного развёртывания и проверки RPi настройте Keenetic, чтобы он объявлял RPi как шлюз для LAN:
...
**DNS для разрешения доменных имён:** Чтобы в `vpn-status.sh` отображались имена доменов, также установите DNS-сервер в Keenetic:
...
**Откат:** Очистите поле «Адрес шлюза» в Keenetic (установите обратно пустое значение или `192.168.1.1`).
...
Если в столбце DOMAIN отображаются IP-адреса, установите DNS на Keenetic равным `192.168.1.254` (см. раздел «Требования и подготовка»).
...
`dnsmasq` на RPi (`192.168.1.254`) должен быть установлен как DNS-сервер в Keenetic, чтобы разрешение доменных имён работало в `vpn-status.sh`. Без этого все DNS-запросы уходят напрямую к вышестоящему резолверу, минуя журнал запросов dnsmasq, и в столбце DOMAIN будут отображаться только IP-адреса.
...
Верните шлюз DHCP на Keenetic обратно на `192.168.1.1`:
...
**Веб-интерфейс Keenetic / приложение Keenetic недоступны с устройств LAN**

Симптом: Невозможно открыть `http://192.168.1.1` с устройств LAN после настройки RPi как шлюза.

Причина: Неограниченное правило MASQUERADE на `eth0` перезаписывает исходные IP-адреса для всего исходящего трафика — включая трафик внутри LAN к адресу `192.168.1.1`. Keenetic видит все запросы как исходящие от `192.168.1.254` и блокирует их.
...
Причина A: dnsmasq не настроен как DNS-сервер в Keenetic — DNS-запросы обходят RPi полностью, поэтому у dnsmasq нет журнала для сопоставления. (Это напрямую не подавляет записи `[VPN]`/`[ISP]`, но стоит проверить в первую очередь.)

Причина B: LAN-устройство не обновило DHCP-аренду после смены шлюза в Keenetic — оно всё ещё использует старый шлюз (например, `192.168.1.1` напрямую), и трафик не проходит через RPi.

Решение: В Keenetic: Домашняя сеть → Сегменты → По умолчанию → Параметры IP → установите «Адрес шлюза» равным `192.168.1.254`. Затем обновите DHCP-аренду на LAN-устройстве (переподключите Wi-Fi или выполните `ipconfig /renew` на Windows). Также установите DNS-сервер равным `192.168.1.254` на этой же странице настроек.
...
Решение: Установите DNS-сервер в Keenetic равным `192.168.1.254` (см. «Требования и подготовка» → DHCP на Keenetic). После обновления аренды DNS-запросы LAN начнут проходить через dnsmasq, который журналирует их для сопоставления.

---

**Маршруты раздельной маршрутизации пропадают после перезагрузки Keenetic (смена carrier в NM)**
...
Причина: Когда Keenetic перезагружается, канал eth0 пропадает (событие смены carrier). NetworkManager (NM) сбрасывает все маршруты eth0 при событии опускания канала — включая ~1360 маршрутов RU-подсетей и маршрут к VPN-серверу, добавленных `routing.sh`. Когда eth0 поднимается снова, NM восстанавливает только локальный маршрут для канала. Без маршрута к VPN-серверу (`84.32.100.60/32 via 192.168.1.1`) команда `ip route get 84.32.100.60` разрешается через `awg0` (таблица политик 51820), создавая петлю маршрутизации. Нет VPN → нет интернета → ежедневная загрузка через cron тоже не работает → система не может самовосстановиться без вмешательства.
